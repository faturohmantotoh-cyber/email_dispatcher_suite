...
// === Action: ADD MANUAL CONTACT ===
if (isset($_POST['add_manual_contact']) && $_POST['add_manual_contact'] === '1') {
    $name = trim($_POST['manual_name'] ?? '');
    $email = trim($_POST['manual_email'] ?? '');

    if ($name === '' || $email === '') {
        $msg = '<span style="color:red">✗ <strong>Nama dan Email wajib diisi.</strong></span>';
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $msg = '<span style="color:red">✗ <strong>Format email tidak valid.</strong></span>';
    } else {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO contacts(display_name, email, source, last_synced)
            ...
            ");
            $stmt->execute([$name, $email]);
            $msg = "✓ <strong>Kontak berhasil ditambahkan!</strong> $name ($email)<script>sessionStorage.setItem('scrollToKontak', '1');</script>";
        } catch (Exception $e) {
            $msg = '<span style="color:red">✗ Gagal menambah kontak: ' . e($e->getMessage()) . '</span>';
        }
    }
}

// === Action: AJAX ADD CONTACT from modal ===
if (isset($_POST['ajax_add_contact']) && $_POST['ajax_add_contact'] === '1') {
    ...
    try {
        ...
        // Get the contact ID (either newly inserted or existing)
        $contactId = $pdo->lastInsertId();
        if (!$contactId) {
            // Contact already exists, fetch its ID
            $sel = $pdo->prepare("SELECT id FROM contacts WHERE email = ?");
            $sel->execute([$email]);
            $contactId = $sel->fetchColumn();
        }

        // Fetch the complete contact data
        $sel = $pdo->prepare("SELECT id, display_name, email, source, last_synced, id as contact_id FROM contacts WHERE id = ?");
        $sel->execute([$contactId]);
        $contact = $sel->fetch(PDO::FETCH_ASSOC);
...
if (isset($_GET['action']) && $_GET['action'] === 'get_group_members') {
    ...
    try {
        ...
        // Get member IDs and their contact details
        ...
        // Get contact details for these members
        $memberDetails = [];
        if (!empty($memberIds)) {
            $placeholders = implode(',', array_fill(0, count($memberIds), '?'));
            $stmt = $pdo->prepare("SELECT `id`, `display_name`, `email` FROM `contacts` WHERE `id` IN ($placeholders)");
...
// Contacts
$rows = $pdo->query("
    SELECT id, display_name, email, source, last_synced, id as contact_id
    FROM contacts
...
header { background: #0052CC; color: white; padding: 1.5rem 2rem; margin: 0; box-shadow: 0 4px 12px rgba(0, 82, 204, 0.15); position: relative; z-index: 1001; }
...
#currentMembersList:empty::before {
  content: 'Belum ada anggota';
  display: block;
  color: #999;
  font-style: italic;
  padding: 1rem;
...
<main>
  ...
  <div class="card card-collapsible" id="cardKontak">
    ...
    <div class="card-toolbar">
      ...
      <button class="btn" type="button" onclick="toggleAddContactForm()" style="margin-left:auto;padding:0.5rem 1rem;font-size:0.85rem;">➕ Tambah Kontak</button>
    </div>

    <!-- Manual Add Contact Form -->
    <div id="addContactForm" style="display:none;padding:1rem;background:#F5F8FF;border:1px solid #E3F2FD;border-radius:0.6rem;margin-bottom:1rem;">
      <h4 style="margin:0 0 0.75rem 0;color:#0052CC;font-size:0.95rem;">Tambah Kontak Baru</h4>
      <form id="manualContactForm" method="post" style="display:grid;grid-template-columns:1fr 1fr auto;gap:0.5rem;align-items:end;">
        <div>
          <label style="display:block;font-size:0.8rem;color:#666;margin-bottom:0.25rem;">Nama</label>
          <input type="text" name="manual_name" class="input" placeholder="Nama kontak" required style="width:100%;padding:0.5rem;border:1px solid #E8E8E8;border-radius:0.4rem;">
        </div>
        <div>
          <label style="display:block;font-size:0.8rem;color:#666;margin-bottom:0.25rem;">Email</label>
          <input type="email" name="manual_email" class="input" placeholder="email@example.com" required style="width:100%;padding:0.5rem;border:1px solid #E8E8E8;border-radius:0.4rem;">
        </div>
        <div>
          <input type="hidden" name="add_manual_contact" value="1">
          <button type="submit" class="btn" style="padding:0.5rem 1rem;">Simpan</button>
        </div>
      </form>
    </div>
    <div class="table-scroll">
  ...
  <div class="drawer">
    ...
    <div class="drawer-body">
      <form id="modalGroupForm" method="post">
        ...
        <div id="currentMembersSection" style="margin-bottom:1.25rem;display:none;">
          <label style="display:block;font-weight:700;color:#0052CC;margin-bottom:0.5rem;">Anggota Grup Saat Ini:</label>
          <div id="currentMembersList" style="max-height:250px;min-height:100px;overflow-y:auto;border:1px solid #E8E8E8;border-radius:0.6rem;padding:0.75rem;background:#FAFAFA;scrollbar-width:thin;scrollbar-color:#0052CC #E8E8E8;">
  ...
  <div class="drawer">
    ...
    <div class="drawer-body">
      <form id="modalGroupOrderForm" method="post">
        ...
        <div id="currentGroupOrderMembers" style="margin-bottom:1.25rem;display:none;">
          <label style="display:block;font-weight:700;color:#0052CC;margin-bottom:0.5rem;">Anggota Grup Saat Ini:</label>
          <div id="currentGroupOrderList" style="max-height:200px;overflow-y:auto;border:1px solid #E8E8E8;border-radius:0.6rem;padding:0.75rem;background:#FAFAFA;">
            <!-- Current group order members will be populated here -->
          </div>
        </div>
  ...
  <div class="drawer">
    ...
    <div class="drawer-body">
      <form id="modalCCGroupForm" method="post">
        ...
        <div id="currentCCGroupMembers" style="margin-bottom:1.25rem;display:none;">
          <label style="display:block;font-weight:700;color:#0052CC;margin-bottom:0.5rem;">Anggota Grup Saat Ini:</label>
          <div id="currentCCGroupList" style="max-height:200px;overflow-y:auto;border:1px solid #E8E8E8;border-radius:0.6rem;padding:0.75rem;background:#FAFAFA;">
            <!-- Current CC group members will be populated here -->
          </div>
        </div>
  ...
  <div class="modal" style="max-width:500px;">
    ...
    <div class="modal-body">
      ...
      <div id="updateGroupMembers" style="display:none;margin-top:16px;padding-top:16px;border-top:1px solid #E3F2FD;">
        ...
        <div id="currentGroupMembers" style="border:1px solid #E8E8E8;border-radius:6px;max-height:300px;min-height:100px;overflow-y:auto;padding:8px;margin-bottom:16px;scrollbar-width:thin;scrollbar-color:#0052CC #E8E8E8;">
        </div>
...
async function openGroupModal(groupId = null) {
  ...
  // Show/hide current members section
  const currentMembersSection = document.getElementById('currentMembersSection');
  console.log('groupId:', groupId, 'currentMembersSection:', currentMembersSection);
  if (currentMembersSection) {
    currentMembersSection.style.display = groupId ? 'block' : 'none';
    console.log('Section display set to:', currentMembersSection.style.display);
  }
      ...
      if (response.ok) {
        ...
        if (data.members && Array.isArray(data.members)) {
          data.members.forEach(memberId => {
            selectedContacts.add(parseInt(memberId));
          });
        }
        
        // Store member details for display
        if (data.member_details && Array.isArray(data.member_details)) {
          window.currentGroupMembers = data.member_details;
          console.log('Stored member_details:', window.currentGroupMembers);
        } else {
          console.log('No member_details in response, using empty array');
...
async function closeGroupModal() {
  ...
  const hasChanges = selectedContacts.size > 0 || document.getElementById('groupName').value.trim() !== '';
  if (hasChanges) {
...
function renderCurrentMembersList() {
  ...
  console.log('window.currentGroupMembers:', window.currentGroupMembers);

  if (selectedContacts.size === 0) {
    container.innerHTML = '<div style="color:#999;font-style:italic;">Belum ada anggota</div>';
    return;
  }

  // Use stored member_details if available, otherwise try to find in allContacts
  let renderedCount = 0;
  if (window.currentGroupMembers && Array.isArray(window.currentGroupMembers)) {
    console.log('Using currentGroupMembers, count:', window.currentGroupMembers.length);
    window.currentGroupMembers.forEach(function(contact) {
      const contactId = parseInt(contact.id);
      const hasContact = selectedContacts.has(contactId);
      console.log('Checking contact:', contact.display_name, 'id:', contactId, 'in selected:', hasContact);
      if (contact && hasContact) {
        renderedCount++;
        const item = document.createElement('div');
        item.style.cssText = 'padding:0.5rem;margin-bottom:0.5rem;background:white;border:1px solid #E8E8E8;border-radius:0.4rem;display:flex;align-items:center;gap:0.5rem;min-height:40px;visibility:visible;';

        const nameSpan = document.createElement('span');
        nameSpan.style.cssText = 'font-weight:600;color:#0052CC;';
        nameSpan.textContent = contact.display_name || 'Unknown';

        const emailSpan = document.createElement('span');
        emailSpan.style.cssText = 'color:#666;font-size:0.85rem;';
        emailSpan.textContent = '(' + (contact.email || 'No email') + ')';

        const removeBtn = document.createElement('button');
        removeBtn.type = 'button';
        removeBtn.style.cssText = 'margin-left:auto;background:#FADDD1;color:#7F4028;border:none;padding:0.3rem 0.6rem;border-radius:0.3rem;cursor:pointer;font-size:0.8rem;font-weight:600;';
        removeBtn.textContent = '\u2717 Hapus';
        removeBtn.onclick = function() { removeMember(parseInt(contact.id)); };
    ...
    console.log('Rendered', renderedCount, 'items from currentGroupMembers');
  } else {
    // Fallback to searching in allContacts
    console.log('Fallback to allContacts for', selectedContacts.size, 'contacts');
    let renderedCount = 0;
    selectedContacts.forEach(function(contactId) {
      const contact = allContacts.find(function(c) { return parseInt(c.id) == contactId || parseInt(c.contact_id) == contactId; });
      console.log('Contact for ID', contactId, ':', contact);
      if (contact) {
        renderedCount++;
        const item = document.createElement('div');
        item.style.cssText = 'padding:0.5rem;margin-bottom:0.5rem;background:white;border:1px solid #E8E8E8;border-radius:0.4rem;display:flex;align-items:center;gap:0.5rem;min-height:40px;visibility:visible;';
        ...
        removeBtn.style.cssText = 'margin-left:auto;background:#FADDD1;color:#7F4028;border:none;padding:0.3rem 0.6rem;border-radius:0.3rem;cursor:pointer;font-size:0.8rem;font-weight:600;';
        removeBtn.textContent = '\u2717 Hapus';
        removeBtn.onclick = function() { removeMember(contactId); };
    ...
    console.log('Rendered', renderedCount, 'items from fallback');
  }
  console.log('=== renderCurrentMembersList done, container childElementCount:', container.childElementCount, '===');
}

// Remove member from selected contacts
...
function renderContactList() {
  ...
  allContacts.forEach(contact => {
    ...
    input.checked = selectedContacts.has(parseInt(contact.id));
    input.onchange = (e) => {
      if (e.target.checked) {
        selectedContacts.add(parseInt(contact.id));
      } else {
        selectedContacts.delete(parseInt(contact.id));
      }
      updateMemberCount();
    };
  ...
  // Show "Add New Contact" option if search returns no results and search term is valid email
  if (matchCount === 0 && searchTerm) {
    const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(searchTerm);

    const noResultDiv = document.createElement('div');
    noResultDiv.style.cssText = 'padding:1.5rem;text-align:center;background:#FFF3E0;border:2px dashed #FF9800;border-radius:0.6rem;margin:0.5rem;';

    const messageP = document.createElement('p');
    messageP.style.cssText = 'color:#E65100;margin:0 0 1rem 0;font-weight:500;';
    messageP.innerHTML = '&#128269; Tidak ada kontak yang cocok dengan pencarian';

    noResultDiv.appendChild(messageP);

    if (isEmail) {
      const addBtn = document.createElement('button');
      addBtn.type = 'button';
      addBtn.className = 'btn success';
      addBtn.style.cssText = 'padding:0.75rem 1.5rem;font-size:0.9rem;';
      addBtn.innerHTML = '&#9997; Tambah Kontak Baru: <strong>' + searchTerm + '</strong>';
      addBtn.onclick = () => addNewContactFromModal(searchTerm, searchTerm);
      noResultDiv.appendChild(addBtn);

      const hintP = document.createElement('p');
      hintP.style.cssText = 'color:#666;font-size:0.8rem;margin-top:0.75rem;';
      hintP.textContent = 'Klik tombol di atas untuk menyimpan kontak baru';
      noResultDiv.appendChild(hintP);
    } else {
      const hintP = document.createElement('p');
      hintP.style.cssText = 'color:#666;font-size:0.85rem;margin:0;';
      hintP.textContent = 'Coba cari dengan nama atau email yang berbeda';
      noResultDiv.appendChild(hintP);
    }
...
// Add new contact from modal via AJAX
async function addNewContactFromModal(email, name) {
  const result = await Swal.fire({
    title: 'Tambah Kontak Baru?',
    html: `<p>Email: <strong>${email}</strong></p><p style="color:#666;font-size:0.85rem;">Kontak akan langsung ditambahkan ke grup yang sedang dibuat.</p>`,
    icon: 'question',
    showCancelButton: true,
  ...
  try {
    ...
    formData.append('ajax_add_contact', '1');
    formData.append('manual_name', name);
    formData.append('manual_email', email);
    ...
    if (data.success && data.contact) {
      // Add to allContacts array
      allContacts.push(data.contact);
      // Auto-select the new contact
      selectedContacts.add(parseInt(data.contact.id));
      ...
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: `Kontak ${data.contact.display_name} telah ditambahkan dan dipilih.`,
        timer: 2000,
...
// Add new contact from CC Group modal via AJAX
async function addNewContactFromModalCC(email, name) {
  const result = await Swal.fire({
    title: 'Tambah Kontak Baru?',
    html: `<p>Email: <strong>${email}</strong></p><p style="color:#666;font-size:0.85rem;">Kontak akan langsung ditambahkan ke grup CC yang sedang dibuat.</p>`,
    icon: 'question',
    showCancelButton: true,
    confirmButtonText: 'Ya, Tambahkan',
    cancelButtonText: 'Batal',
  ...
  try {
    ...
    if (data.success && data.contact) {
      ...
      allContacts.push(data.contact);
      // Auto-select the new contact
      selectedCCContacts.add(parseInt(data.contact.id));
      // Clear search and re-render
      document.getElementById('ccContactSearch').value = '';
      renderCCContactList();
      updateCCMemberCount();
      renderCurrentCCGroupList();
...
// Close modal when clicking outside - with confirmation if unsaved changes
document.getElementById('groupModal')?.addEventListener('click', async function(e) {
  if (e.target === this) {
    // Check if there are unsaved changes
    const hasChanges = selectedContacts.size > 0 || document.getElementById('groupName').value.trim() !== '';
    if (hasChanges) {
...
async function openGroupOrderModal(groupOrderId = null) {
  ...
  // Show/hide current members section
  const currentMembersSection = document.getElementById('currentGroupOrderMembers');
  if (currentMembersSection) {
    currentMembersSection.style.display = groupOrderId ? 'block' : 'none';
  }

  renderGroupList();
  updateGroupOrderCount();
  renderCurrentGroupOrderList();
...
async function closeGroupOrderModal() {
  // Check if there are unsaved changes
  const hasChanges = selectedGroups.size > 0 || document.getElementById('groupOrderName').value.trim() !== '';
  if (hasChanges) {
    const result = await Swal.fire({
      title: 'Tutup Form?',
      text: 'Anda memiliki perubahan yang belum disimpan. Apakah Anda yakin ingin menutup?',
...
// Render current group order list
function renderCurrentGroupOrderList() {
  const container = document.getElementById('currentGroupOrderList');
  if (!container) return;
  ...
  if (selectedGroups.size === 0) {
    container.innerHTML = '<div style="color:#999;font-style:italic;">Belum ada grup</div>';
    return;
  }
  ...
  selectedGroups.forEach(function(groupId) {
    ...
    if (group) {
      const item = document.createElement('div');
      item.style.cssText = 'padding:0.5rem;margin-bottom:0.5rem;background:white;border:1px solid #E8E8E8;border-radius:0.4rem;display:flex;align-items:center;gap:0.5rem;';

      const nameSpan = document.createElement('span');
      nameSpan.style.cssText = 'font-weight:600;color:#0052CC;';
      nameSpan.textContent = group.name || 'Unknown';

      const memberSpan = document.createElement('span');
      memberSpan.style.cssText = 'color:#666;font-size:0.85rem;';
      memberSpan.textContent = '(' + (group.members || 0) + ' anggota)';

      const removeBtn = document.createElement('button');
      removeBtn.type = 'button';
      removeBtn.style.cssText = 'margin-left:auto;background:#FADDD1;color:#7F4028;border:none;padding:0.3rem 0.6rem;border-radius:0.3rem;cursor:pointer;font-size:0.8rem;font-weight:600;';
      removeBtn.textContent = '\u2717 Hapus';
      removeBtn.onclick = function() { removeGroupFromOrder(groupId); };
...
// Close modal when clicking outside - with confirmation if unsaved changes
document.getElementById('groupOrderModal')?.addEventListener('click', async function(e) {
  if (e.target === this) {
    // Check if there are unsaved changes
    const hasChanges = selectedGroups.size > 0 || document.getElementById('groupOrderName').value.trim() !== '';
    if (hasChanges) {
    ...
    .then(data => {
      if (data.members) {
        data.members.forEach(member => {
          selectedContactsForUpdate.add(parseInt(member.contact_id));
        });
      }
...
function renderCurrentGroupMembers() {
  ...
  selectedContactsForUpdate.forEach(function(contactId) {
    ...
    if (contact) {
      ...
      const nameDiv = document.createElement('div');
      const nameDivInner1 = document.createElement('div');
      nameDivInner1.style.cssText = 'font-weight:600;color:#0052CC;';
      nameDivInner1.textContent = contact.display_name;
      const nameDivInner2 = document.createElement('div');
      nameDivInner2.style.cssText = 'font-size:12px;color:#666;';
      nameDivInner2.textContent = contact.email;
      nameDiv.appendChild(nameDivInner1);
      nameDiv.appendChild(nameDivInner2);
...
function renderAvailableContacts() {
  ...
  const filtered = allContacts.filter(function(contact) {
    if (selectedContactsForUpdate.has(parseInt(contact.id))) return false;
    const searchStr = contact.display_name + ' ' + contact.email;
    return searchStr.toLowerCase().includes(searchTerm);
  });
  ...
  filtered.forEach(function(contact) {
    ...
    addBtn.onclick = function() {
      selectedContactsForUpdate.add(parseInt(contact.id));
      renderCurrentGroupMembers();
      renderAvailableContacts();
    };
...
function toggleSelectAllUpdate(selectAll) {
  ...
  allContacts.forEach(contact => {
    ...
    if (selectAll && selectedContactsForUpdate.has(parseInt(contact.id))) return;
    ...
    if (selectAll) {
      ...
      selectedContactsForUpdate.delete(parseInt(contact.id));
    }
  });
...
async function openCCGroupModal(ccGroupId = null) {
  ...
  // Show/hide current members section
  const currentMembersSection = document.getElementById('currentCCGroupMembers');
  if (currentMembersSection) {
    currentMembersSection.style.display = ccGroupId ? 'block' : 'none';
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data.member_ids)) {
          data.member_ids.forEach(id => selectedCCContacts.add(parseInt(id)));
        }
        renderCCContactList();
        updateCCMemberCount();
        renderCurrentCCGroupList();
...
async function closeCCGroupModal() {
  // Check if there are unsaved changes
  const hasChanges = selectedCCContacts.size > 0 || document.getElementById('ccGroupName').value.trim() !== '';
  if (hasChanges) {
    const result = await Swal.fire({
      title: 'Tutup Form?',
      text: 'Anda memiliki perubahan yang belum disimpan. Apakah Anda yakin ingin menutup?',
...
function renderCCContactList() {
  const searchTerm = document.getElementById('ccContactSearch').value.toLowerCase().trim();
  ...
  allContacts.forEach(contact => {
    ...
    input.type = 'checkbox';
    input.value = contact.id;
    input.checked = selectedCCContacts.has(parseInt(contact.id));
    input.onchange = (e) => {
      if (e.target.checked) selectedCCContacts.add(parseInt(contact.id));
      else selectedCCContacts.delete(parseInt(contact.id));
      updateCCMemberCount();
      renderCurrentCCGroupList();
    };
  ...
  if (matchCount === 0 && searchTerm) {
    ...
    if (isEmail) {
      ...
      addBtn.style.cssText = 'padding:0.75rem 1.5rem;font-size:0.9rem;';
      addBtn.innerHTML = '&#9997; Tambah Kontak Baru: <strong>' + searchTerm + '</strong>';
      addBtn.onclick = () => addNewContactFromModalCC(searchTerm, searchTerm);
      noResultDiv.appendChild(addBtn);
...
function toggleSelectAllCC(selectAll) {
  ...
  allContacts.forEach(contact => {
    ...
    if (!searchStr.includes(searchTerm)) return;
    if (selectAll) selectedCCContacts.add(parseInt(contact.id));
    else selectedCCContacts.delete(parseInt(contact.id));
  });
  renderCCContactList();
  updateCCMemberCount();
...
// Render current CC group list
function renderCurrentCCGroupList() {
  const container = document.getElementById('currentCCGroupList');
  if (!container) return;
  ...
  selectedCCContacts.forEach(function(contactId) {
    const contact = allContacts.find(function(c) { return c.id == contactId; });
    if (contact) {
      const item = document.createElement('div');
      item.style.cssText = 'padding:0.5rem;margin-bottom:0.5rem;background:white;border:1px solid #E8E8E8;border-radius:0.4rem;display:flex;align-items:center;gap:0.5rem;';
      ...
      removeBtn.style.cssText = 'margin-left:auto;background:#FADDD1;color:#7F4028;border:none;padding:0.3rem 0.6rem;border-radius:0.3rem;cursor:pointer;font-size:0.8rem;font-weight:600;';
      removeBtn.textContent = '\u2717 Hapus';
      removeBtn.onclick = function() { removeContactFromCCGroup(contactId); };
...
// Remove contact from CC group
function removeContactFromCCGroup(contactId) {
  selectedCCContacts.delete(contactId);
  renderCCContactList();
  updateCCMemberCount();
  renderCurrentCCGroupList();
}
...
// Close modal when clicking outside - with confirmation if unsaved changes
document.getElementById('ccGroupModal')?.addEventListener('click', async function(e) {
  if (e.target === this) {
    // Check if there are unsaved changes
    const hasChanges = selectedCCContacts.size > 0 || document.getElementById('ccGroupName').value.trim() !== '';
    if (hasChanges) {
...
function toggleAddContactForm() {
  ...
  if (form.style.display === 'none') {
    ...
    document.querySelector('#addContactForm input[name="manual_name"]').focus();
  } else {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Scroll to contact list after successful manual contact addition
  if (sessionStorage.getItem('scrollToKontak') === '1') {
    sessionStorage.removeItem('scrollToKontak');
    const cardKontak = document.getElementById('cardKontak');
    if (cardKontak) {
      // Expand the card if collapsed first
      cardKontak.classList.remove('collapsed');
      localStorage.setItem('card_cardKontak', '0');
      // Wait for expansion animation to complete then scroll smoothly
      setTimeout(function() {
        const headerOffset = 100;
        const elementPosition = cardKontak.getBoundingClientRect().top + window.pageYOffset;
        const offsetPosition = elementPosition - headerOffset;
        window.scrollTo({
          top: offsetPosition,
          behavior: 'smooth'
        });
        // Flash highlight the card briefly to show where we landed
        cardKontak.style.transition = 'box-shadow 0.5s ease';
        cardKontak.style.boxShadow = '0 0 20px rgba(0, 82, 204, 0.4)';
        setTimeout(function() {
          cardKontak.style.boxShadow = '';
        }, 1000);
      }, 400);
    }
  }
