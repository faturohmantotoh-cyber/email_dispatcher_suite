...
$useAIMatching = ($_POST['use_ai_matching'] ?? '0') === '1';
 ...
 * @param bool $useAI Whether to use Groq AI for matching
 * @return float Similarity score 0-100
 */
function enhanced_similarity($filename, $email, $name, $recipientGroups, $useAI = false) {
    // Base similarity: compare filename with email + name
    if ($useAI) {
        $score1 = similarity_score_enhanced($filename, $email, $email, true);
        $score2 = similarity_score_enhanced($filename, $name, $email, true);
    } else {
    ...
    if (!empty($recipientGroups)) {
        ...
        foreach ($recipientGroups as $g) {
            ...
            if ($groupName) {
                ...
                if ($useAI) {
                    $groupScore = similarity_score_enhanced($filename, $groupName, $email, true);
                } else {
...
if (!empty($selectedGroupIds)) {
    ...
    foreach ($groupMembers as $groupId => $members) {
        ...
        foreach ($attachments as $a) {
            $score = enhanced_similarity($a['filename'], $primaryMember['email'], $primaryMember['name'], $primaryMember['groups'], $useAIMatching);
    ...
    foreach ($ungroupedRecips as $r) {
        ...
        foreach ($attachments as $a) {
            $score = enhanced_similarity($a['filename'], $email, $name, $recipientGroups, $useAIMatching);
            if ($score > $bestScore) {
                $bestScore = $score;
                $best = $a;
                $bestPath = $a['path'];
